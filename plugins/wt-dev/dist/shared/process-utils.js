 ...
 */
export async function waitForPortRelease(port, targetPids, maxRetries = MAX_PORT_CHECK_RETRIES) {
    for (let i = 0; i < maxRetries; i++) {
        ...
        if (targetPids.size > 0) {
            ...
            if (!targetStillPresent && currentPids.length > 0) {
                ...
                throw new Error(`Port ${port} re-bound by different process (PID ${currentPids[0]}) during cleanup`);
            }
        }
