...
import fs from 'fs';
import { loadConfig, getDevServerLogPath } from '../config.js';
 ...
 */
export async function showDevServerStatus(config) {
 ...
 * Stop the dev server
 */
export async function stopDevServer(config) {
    const cfg = config ?? loadConfig();
    const wtConfig = getWorktreeConfig(cfg);
...
export async function startDevServer(config, options = {}) {
    ...
    if (portInUse) {
        if (options.force) {
            console.log(`${colors.YELLOW}⚠ Dev server is already running on port ${wtConfig.port}${colors.NC}`);
 ...
 */
export function showDevServerLogs(config) {
    const cfg = config ?? loadConfig();
