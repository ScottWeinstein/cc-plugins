 ...
 */
function computeHashPort(worktreePath, ports) {
 ...
 */
function findContainingWorktree(projectPath, worktrees) {
    let projectRealPath;
    try {
 ...
 */
function computeSecondaryHashPort(worktreePath, ports, attempt) {
 ...
 */
function resolveAllPorts(worktrees, ports) {
    ...
    for (const wt of worktrees) {
        ...
        for (let attempt = 0; attempt < maxAttempts; attempt++) {
            const candidatePort = attempt === 0 ? computeHashPort(wt, ports) : computeSecondaryHashPort(wt, ports, attempt);
        ...
        if (!assignments.has(wt)) {
            throw new Error(`Port pool exhausted: ${worktrees.length} worktrees but only ${ports.length} ports available. ` +
 ...
 */
export function getWorktreePort(projectRoot, ports) {
